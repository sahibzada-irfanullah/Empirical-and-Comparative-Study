...
#     print(["{0:.2f}".format(acc), "{0:.2f}".format(prec), "{0:.2f}".format(recal), "{0:.2f}".format(f1)])
  ...
  # def testCode(self, y_original, y_reshape):
  # '''test code for whether the y original and y rehaped is equal or not.. test satus is passed'''
  #   count = 0
  #   for i in range(0, y_reshape.shape[0]):
  #     if(y_reshape[i] == y_original[i]):
  #       count = count + 1
  ...
  def preparFeatLabelsPhase2(self, X, y_original, y_predicted):
    print("under process")
    print("actual y")
    ...
    print(y_predicted)
    print("indices")
    indices = np.nonzero(y_predicted == 0)[0]
    print(indices)
    print(indices.shape)
    upX = self.delete_rows_csr(X, indices)
    print("up X info")
    print(upX.toarray())
    print(X.toarray())
    print(type(upX))
  ...
  def training_clfs(self, encap_res, y_binary, y_multi, k_fold = 2, pathToResFolder = None, pathToClfFolder = None):
    ...
    # unigrams
    #phase 1
    df = self.stratified_cv(uniTrainDtm, y_binary, uniSpecs, save_clf, n_splits=k_fold)
    xl.save_resultsToExcel(df, self.extractInfo_specs(uniSpecs, 'sheet_name'),
                           uniSpecs, self.extractInfo_specs(uniSpecs, 'freq'))
    #phase 2

    # uniBigrams
    df = self.stratified_cv(uniBiTrainDtm, y_binary, uniBiSpecs, save_clf, n_splits=k_fold)
    xl.save_resultsToExcel(df, self.extractInfo_specs(uniBiSpecs, 'sheet_name')
                           , uniBiSpecs, self.extractInfo_specs(uniBiSpecs, 'freq'))
    # uniBiTrigrams
    df = self.stratified_cv(uniBiTriTrainDtm, y_binary, uniBiTriSpecs, save_clf, n_splits=k_fold)
    ...
    # bigrams
    df = self.stratified_cv(biTrainDtm, y_binary, biSpecs, save_clf, n_splits=k_fold)
    xl.save_resultsToExcel(df, self.extractInfo_specs(biSpecs, 'sheet_name')
                           , biSpecs, self.extractInfo_specs(biSpecs, 'freq'))
    # biTrigrams
    df = self.stratified_cv(biTriTrainDtm, y_binary, biTriSpecs, save_clf, n_splits=k_fold)
    xl.save_resultsToExcel(df, self.extractInfo_specs(biTriSpecs, 'sheet_name')
                           , biTriSpecs, self.extractInfo_specs(biTriSpecs, 'freq'))
    # trigrams
    df = self.stratified_cv(triTrainDtm, y_binary, triSpecs, save_clf, n_splits=k_fold)

...
class Training_Classifiers_ML:
  ...
  def stratified_cv(self, value, pathToResultFilename, save_clf, n_splits=5, shuffle=True):
  ...
  def create_Folder(self, file_path):
    ...
    try:
      ...
      if e.errno != errno.EEXIST:
        raise
  def training_clfs(self, encap_res, le_name_map_bin, k_fold = 2, featureTypeFolderLabel= None):
    ...
    if  featureTypeFolderLabel.lower() in ["simple", "tfidf", "tf-idf", "vect","vec"]:
      for key,value in encap_res.items():
        ...
        self.stratified_cv(value, pathTofileName, save_clf, n_splits=k_fold, shuffle=True)
        gc.collect()
